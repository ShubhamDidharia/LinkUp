...
const Post = ({ post }) => {
  ...
  return (
    <article className="bg-[#1A1A1A] hover:bg-[#1A1A1A] transition-all duration-200 p-6 group border-b border-[#2A2A2A]">
      <div className="flex gap-4 items-start">
        {/* Avatar */}
        <Link to={`/profile/${postOwner.username}`} className="flex-shrink-0">
          <div className="w-12 h-12 rounded-full overflow-hidden ring-2 ring-[#2A2A2A] shadow-md hover:ring-[#E8450A] transition-all duration-200">
            <img
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mb-3">
            <Link
              ...
              className="font-semibold text-white hover:text-[#E8450A] transition-colors"
            >
              {postOwner.fullName}
            </Link>
            <Link
          ...
          {/* Edit Mode */}
          {editMode ? (
            <div className="mb-4 p-4 bg-[#2A2A2A] rounded-xl border border-[#2A2A2A]">
              <textarea
                value={editText}
                onChange={(e) => setEditText(e.target.value)}
                className="w-full p-3 border border-[#2A2A2A] bg-[#1A1A1A] text-white rounded-lg focus:outline-none focus:ring-2 focus:ring-[#E8450A] resize-none transition-colors"
                rows={4}
              />
              {editImg && (
                <div className="mt-3 mb-3 rounded-lg overflow-hidden border border-[#2A2A2A]">
              ...
              <div className="flex gap-2 justify-end mt-3">
                <button
                  ...
                  className="px-4 py-2 bg-[#2A2A2A] text-white rounded-full font-medium hover:bg-[#333333] transition-colors border border-[#2A2A2A]"
                >
                  Cancel
                </button>
                <button
                  ...
                  disabled={isUpdating}
                  className="px-4 py-2 bg-[#E8450A] text-white rounded-full font-medium hover:bg-[#c83c0a] transition-colors disabled:opacity-50"
                >
            ...
            <>
              ...
              {/* Post Image */}
              {post.img && (
                <div className="relative mb-4 rounded-2xl overflow-hidden border border-[#2A2A2A]">
                  <img
                    src={post.img}
              ...
              {(post.isNSFW || post.isBlurred) && showNsfw && (
                <div className="mt-4 text-center">
                  <button
                    ...
                    className="px-4 py-2 bg-[#2A2A2A] text-white rounded-full text-sm font-medium hover:bg-[#333333] transition-colors"
                  >
                    Hide NSFW Content
                  </button>
                </div>
          ...
          <div className="flex items-center justify-between pt-2">
            <div className="flex items-center gap-8">
              {/* Comments */}
              <button
                className="flex items-center gap-2 text-[#888888] hover:text-[#E8450A] transition-colors group/comment"
                onClick={() => document.getElementById("comments_modal" + post._id).showModal()}
              >
                <div className="p-2 rounded-full group-hover/comment:bg-[#1A1A1A] transition-colors">
              ...
              {/* Repost */}
              <button className="flex items-center gap-2 text-[#888888] hover:text-green-500 transition-colors group/repost">
                <div className="p-2 rounded-full group-hover/repost:bg-[#1A1A1A] transition-colors">
              ...
              <button
                ...
                className={`flex items-center gap-2 transition-colors group/like ${
                  isLiked ? "text-red-500" : "text-[#888888] hover:text-red-500"
                }`}
                disabled={isLiking}
              >
                <div className="p-2 rounded-full group-hover/like:bg-[#1A1A1A] transition-colors\">
                  {isLiking ? (
                    <LoadingSpinner size="sm" />
                  ) : isLiked ? (
            ...
            <button
              ...
              className={`p-2 rounded-full transition-colors ${
                isBookmarked
                  ? "text-[#E8450A] hover:text-[#c83c0a] hover:bg-[#1A1A1A]"
      ...
      <dialog id={`comments_modal${post._id}`} className="modal">
        <div className="modal-box bg-[#111111] rounded-2xl border border-[#2A2A2A] shadow-xl max-w-2xl transition-colors">
          ...
          <div className="space-y-4 max-h-60 overflow-auto mb-6">
            {post.comments.length === 0 && (
              <div className="text-center py-8">
                ...
                <p className="text-[#888888]">No comments yet. Be the first to comment!</p>
              </div>
            )}

            {post.comments.map((comment) => (
              <div key={comment._id} className="flex gap-3 p-3 rounded-xl hover:bg-[#2A2A2A] transition-colors">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1 w-full">
                    ...
                    <span className="text-[#888888] text-sm">@{comment.user.username}</span>
          ...
          <form onSubmit={handlePostComment} className="border-t border-[#2A2A2A] pt-4">
            <div className="flex gap-3">
              ...
              <div className="flex-1">
                <div className="bg-[#2A2A2A] rounded-xl p-3 border border-[#2A2A2A] focus-within:bg-[#2A2A2A] focus-within:ring-2 focus-within:ring-[#E8450A] focus-within:border-transparent transition-all duration-200">
                  <textarea
                    className="w-full text-white placeholder-[#888888] resize-none border-none focus:outline-none bg-transparent"
                ...
                <div className="flex justify-end mt-3">
                  <button
                    ...
                    className="px-6 py-2 bg-[#E8450A] text-white rounded-full font-medium hover:bg-[#c83c0a] transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                  >
