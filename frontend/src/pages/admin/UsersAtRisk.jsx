            ...
            >
                ...
                <div className="p-5 space-y-4">
                    ...
                    <div className="flex flex-wrap gap-3 text-xs text-slate-500 border-t border-slate-800 pt-3">
                        ...
                        {post.isNSFW && (
                            <span className="px-2 py-0.5 bg-rose-500/10 text-rose-400 border border-rose-500/20 rounded-full font-bold">
                                NSFW Tagged
                            </span>
                        )}
                        {post.autoFlagged && (
                            <span className="px-2 py-0.5 bg-amber-500/10 text-amber-400 border border-amber-500/20 rounded-full font-bold">
                                Auto-Flagged
                            </span>
                        )}
                        {post.flagReasons?.length > 0 && (
                            <span className="text-amber-400">⚑ {post.flagReasons.join(", ")}</span>
                        )}
                    </div>
                </div>
            </div>
...
const UserHistoryDrawer = ({ user, onClose }) => {
    ...
    return (
        <div className="mt-3 bg-slate-950 border border-slate-800 rounded-2xl overflow-hidden">
            {/* Drawer header */}
            ...
            <div className="flex border-b border-slate-800">
                <button
                    ...
                    className={`flex-1 py-2.5 text-xs font-bold uppercase tracking-wider flex items-center justify-center gap-2 cursor-pointer transition-colors ${historyTab === "posts" ? "text-indigo-400 border-b-2 border-indigo-500 bg-slate-900/40" : "text-slate-500 hover:text-slate-300"}`}
                >
                    <FaFileAlt /> Posts ({posts.length})
                </button>
                <button
                    onClick={() => setHistoryTab("comments")}
                    className={`flex-1 py-2.5 text-xs font-bold uppercase tracking-wider flex items-center justify-center gap-2 cursor-pointer transition-colors ${historyTab === "comments" ? "text-indigo-400 border-b-2 border-indigo-500 bg-slate-900/40" : "text-slate-500 hover:text-slate-300"}`}
                >
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            {posts.map((post) => (
                                <button
                                    ...
                                    className="bg-slate-900 border border-slate-800 hover:border-indigo-500/40 rounded-xl p-3 text-left transition-all duration-200 cursor-pointer group"
                                >
            ...
            {user.role !== "admin" && (
                <div className="flex gap-2 p-4 border-t border-slate-800 bg-slate-900/50">
                    <button
                        disabled={actionMutation.isPending}
                        onClick={() => actionMutation.mutate("warn")}
                        className="btn btn-sm bg-amber-600 hover:bg-amber-700 border-none text-white rounded-xl px-4 cursor-pointer flex items-center gap-1.5 disabled:opacity-40"
                    >
                    ...
                    {user.status !== "suspended" && (
                        <button
                            ...
                            className="btn btn-sm bg-rose-700 hover:bg-rose-800 border-none text-white rounded-xl px-4 cursor-pointer flex items-center gap-1.5 disabled:opacity-40"
                        >
                    ...
                    {(user.status === "suspended" || user.strikes > 0) && (
                        <button
                            ...
                            className="btn btn-sm bg-emerald-700 hover:bg-emerald-800 border-none text-white rounded-xl px-4 cursor-pointer flex items-center gap-1.5 disabled:opacity-40"
                        >
                ...
                <div className="flex items-center gap-3 shrink-0">
                    <div className="hidden sm:flex flex-col items-end gap-1">
                        <div className="flex items-center gap-1.5 text-xs text-slate-400">
                            <FaEnvelope className="text-indigo-400/60 text-[10px]" />
                            <span className="truncate max-w-32">{user.email}</span>
                        </div>
                        <div className="flex items-center gap-2">
                            <span className="text-[10px] text-rose-400 font-bold">{user.strikes} strikes</span>
                            <span className="text-[10px] text-slate-500">·</span>
                            <span className="text-[10px] text-amber-400">{user.autoFlaggedPosts} flagged posts</span>
                        </div>
                    </div>
                    <span className={`px-2.5 py-1 rounded-full text-[10px] font-black uppercase border ${user.status === "suspended" ? "bg-rose-500/10 text-rose-400 border-rose-500/20" : "bg-amber-500/10 text-amber-400 border-amber-500/20"}`}>
                        {user.status}
                    </span>
    ...
    return (
        <div className="p-8 space-y-6 text-slate-100 animate-fade-in">
            ...
            <div className="border-b border-slate-900 pb-6">
                <h1 className="text-3xl font-extrabold tracking-tight bg-gradient-to-r from-rose-400 via-amber-400 to-orange-400 bg-clip-text text-transparent">
                    Users Overview
                </h1>
            ...
            <div className="inline-flex items-center bg-slate-900 border border-slate-800 p-1 rounded-xl gap-1">
                <button
                    ...
                    className={`px-5 py-2 rounded-lg text-sm font-bold transition-all duration-200 cursor-pointer flex items-center gap-2 ${activeTab === "suspended" ? "bg-gradient-to-r from-rose-600 to-rose-500 text-white shadow-lg shadow-rose-600/20" : "text-slate-400 hover:text-slate-200"}`}
                >
                ...
                <button
                    ...
                    className={`px-5 py-2 rounded-lg text-sm font-bold transition-all duration-200 cursor-pointer flex items-center gap-2 ${activeTab === "under_review" ? "bg-gradient-to-r from-amber-600 to-amber-500 text-white shadow-lg shadow-amber-600/20" : "text-slate-400 hover:text-slate-200"}`}
                >
            ...
            {isLoading ? (
                ...
                <div className="text-center py-20 bg-slate-900/20 border border-slate-800/40 rounded-2xl">
                    <FaUserCheck className="text-4xl mx-auto mb-4 text-emerald-500/40" />
