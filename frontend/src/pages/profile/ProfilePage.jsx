    ...
    <div className="flex-1 bg-gradient-to-b from-slate-50 dark:from-slate-900/50 to-white dark:to-slate-900 min-h-screen border-x border-slate-200 dark:border-slate-700 transition-colors">
      ...
      {!isLoading && !isRefetching && !user && (
        <div className="flex flex-col items-center justify-center py-20">
          <div className="w-32 h-32 bg-gradient-to-br from-red-100 dark:from-red-900/40 to-pink-100 dark:to-pink-900/40 rounded-full flex items-center justify-center mb-8 shadow-lg">
            <div className="text-5xl">😕</div>
          </div>
          <h3 className="text-2xl font-bold text-slate-900 dark:text-slate-100 mb-3">User not found</h3>
          <p className="text-slate-500 dark:text-slate-400 text-lg">The profile you're looking for doesn't exist.</p>
        </div>
      )}
          ...
          <>
            ...
            <div className="relative group/cover">
              ...
              <div className="absolute -bottom-20 left-8">
                <div className="w-40 h-40 rounded-full relative group/avatar ring-6 ring-white dark:ring-slate-800 shadow-2xl bg-white dark:bg-slate-800 transition-colors">
                  <img
            ...
            <div className="px-8 mt-20 mb-8">
              <div className="bg-white dark:bg-slate-800 rounded-3xl p-8 shadow-lg border border-slate-100 dark:border-slate-700 backdrop-blur-sm transition-colors">
                <div className="flex flex-col gap-6">
                  <div className="flex flex-col gap-2">
                    <div className="flex items-center gap-3">
                      <h2 className="font-bold text-3xl text-slate-900 dark:text-slate-100">{user?.fullName}</h2>
                      <MdVerified className="w-6 h-6 text-blue-500" />
                    </div>
                    <p className="text-slate-500 dark:text-slate-400 text-xl font-medium">@{user?.username}</p>
                    {user?.bio && (
                      <div className="mt-4 p-4 bg-slate-50 dark:bg-slate-700/50 rounded-2xl border border-slate-100 dark:border-slate-600 transition-colors">
                        <p className="text-slate-700 dark:text-slate-300 leading-relaxed text-lg">{user?.bio}</p>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="flex gap-8 flex-wrap">
                    {user?.link && (
                      <div className="flex gap-3 items-center group p-3 rounded-xl hover:bg-blue-50 dark:hover:bg-blue-900/30 transition-colors">
                        <FaLink className="w-5 h-5 text-slate-400 dark:text-slate-500 group-hover:text-blue-500 dark:group-hover:text-blue-400 transition-colors" />
                        <a
                          href="https://youtube.com/@asaprogrammer_"
                          target="_blank"
                          rel="noreferrer"
                          className="text-blue-500 dark:text-blue-400 hover:text-blue-600 dark:hover:text-blue-300 hover:underline transition-colors font-medium"
                        >
                          youtube.com/@asaprogrammer_
                        </a>
                      </div>
                    )}
                    <div className="flex gap-3 items-center p-3 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                      <IoCalendarOutline className="w-5 h-5 text-slate-400 dark:text-slate-500" />
                  ...
                  <div className="flex gap-12 pt-4 border-t border-slate-200 dark:border-slate-700 transition-colors">
                    <div className="flex gap-3 items-center group cursor-pointer">
                      <span className="font-bold text-2xl text-slate-900 dark:text-slate-100 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">
                        {user?.following.length}
                      </span>
                      <span className="text-slate-500 dark:text-slate-400 font-medium group-hover:text-slate-700 dark:group-hover:text-slate-300 transition-colors">
                        Following
                      </span>
                    </div>
                    <div className="flex gap-3 items-center group cursor-pointer">
            ...
            <div className="sticky top-[73px] z-10 bg-white/95 dark:bg-slate-800/95 backdrop-blur-xl border-b border-slate-200 dark:border-slate-700 shadow-sm transition-colors">
